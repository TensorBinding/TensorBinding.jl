...
# many Chebyshev moments, fine spatial/k grids — the regime targeted by the
# SLURM scripts in examples/nontracked/APSOS/) tractable by moving the
...
#
#   σ = TensorBinding.get_conductivity_ward_gpu(H, H, ωlist; Ncheb=80, ...)
#
# ENTRY POINTS (each documented in its own docstring below)
#   Conductivity (experimental/not working yet)
#     get_conductivity_ward_gpu(H1, H2, ωlist; ...)     — Ward-identity σ(ω) (Experimental/not working yet)
#     get_conductivity_cheb2d_gpu(H1, H2, ωlist; ...)   — 2D-Chebyshev σ(ω) (Experimental/not working yet)
...
#
# PRODUCTION SCRIPTS
#   See examples/nontracked/APSOS/*.jl + *.sh for the SLURM driver scripts
#   that exercise this toolkit (magnetic-Hubbard SCF + magnetization/bands
#   maps, spatial LDOS with the :point/:block samplers, ...).
# ============================================================
...
"""
    get_conductivity_ward_gpu(H1, H2, ωlist; kwargs...) -> Vector{ComplexF64}

GPU-accelerated version of `get_conductivity_ward`.

GPU handles: Chebyshev recurrence, density matrix transfer, weighted MPO sums,
             MPO×MPO multiplications, Hadamard products.
CPU handles: replace_sites, QFT sandwich, inner products, per-ω accumulation.

All kwargs are identical to `get_conductivity_ward` (threading is removed —
use julia -t N at the process level instead).
"""
function get_conductivity_ward_gpu(H1::TBHamiltonian, H2::TBHamiltonian,
                                    ωlist::AbstractVector{<:Real};
                                    q_int::Int                = 1,
                                    Lx::Int                   = H1.L ÷ 2,
                                    q_sq::Union{Real,Nothing} = nothing,
                                    Ncheb::Int            = 50,
                                    maxdim::Int           = 200,
                                    cutoff::Real          = 1e-8,
                                    ϵF::Real              = 0.0,
    ...
    _check_gpu("get_conductivity_ward_gpu")
    H1.geometry === nothing && error("get_conductivity_ward_gpu: H1.geometry is not set")
    L1 = H1.L; L2 = H2.L
    @assert L1 == L2 "get_conductivity_ward_gpu: H1 and H2 must have the same L"
    nω = length(ωlist)

    if q_sq === nothing
        q_vec, q_sq_val = _q_int_to_physical(H1, q_int; Lx=Lx)
        q_sq_val = Float64(q_sq_val)
        verbose && println("ward_gpu: q_int=$q_int → |q|²=$(round(q_sq_val; digits=6))")
    else
        q_sq_val = Float64(q_sq)
        verbose && println("ward_gpu: q_int=$q_int, |q|²=$(round(q_sq_val; digits=6)) (provided)")
    end
    (isnan(q_sq_val) || q_sq_val < 1e-14) &&
        error("get_conductivity_ward_gpu: q_sq is NaN or ≈ 0.  Provide q_sq explicitly.")

    v_q = _density_vertex_mps(H1, q_int)

    _ensure_scale!(H1); _ensure_scale!(H2)
    scale1 = H1.scale; center1 = H1.center
    scale2 = H2.scale; center2 = H2.center

    # ── 2D GF coefficients (CPU, small dense matrices) ───────────────────────
    # Computed BEFORE the Tn recurrence so we know which (m,n) pairs matter.
    N = Ncheb + 1
    verbose && println("ward_gpu: 2D GF coefficients for $nω frequencies (CPU)...")
    ...
    elseif kernel != :none
        error("get_conductivity_ward_gpu: unknown kernel=$kernel")
    end

    # Active (m,n) pairs: those where max_ω |C_mn(ω)| exceeds the threshold.
    # This determines which Tns we need — no Tucker SVD required.
    max_C_abs    = reduce((A, B) -> max.(A, abs.(B)), C_all; init=zeros(N, N))
    pair_tol     = tucker_tol * maximum(max_C_abs)
    active_pairs = [(m, n) for m in 1:N for n in 1:N if max_C_abs[m, n] > pair_tol]
    active_m_set = Set(m for (m, _) in active_pairs)
    active_n_set = H1 === H2 ? active_m_set : Set(n for (_, n) in active_pairs)
    verbose && println("ward_gpu: $(length(active_pairs)) active (m,n) pairs " *
                       "($(length(active_m_set)) unique m, $(length(active_n_set)) unique n)")

    # ── Chebyshev recurrence on GPU (only active indices stored) ─────────────
    verbose && println("ward_gpu: Chebyshev recurrence on GPU (Ncheb=$Ncheb)...")
    Tn1, _, _ = KPM_Tn_gpu(H1.mpo, Ncheb, H1.sites;
                            scale=scale1, center=center1,
                            maxdim=maxdim, cutoff=cutoff,
                            keep_indices=active_m_set, verbose=false)
    Tn2 = H1 === H2 ? Tn1 : KPM_Tn_gpu(H2.mpo, Ncheb, H2.sites;
                                         scale=scale2, center=center2,
                                         maxdim=maxdim, cutoff=cutoff,
                                         keep_indices=active_n_set, verbose=false)[1]

    # ── Density matrices: purify on CPU, move to GPU ──────────────────────────
    verbose && println("ward_gpu: density matrices (CPU purification → GPU)...")
    P1_gpu = _get_density_matrix_gpu(H1, ϵF, P_method, Ncheb, maxdim, cutoff,
                                      purify_method, purify_maxdim, purify_maxiters,
                                      purify_tol, verbose)
    P2_gpu = H1 === H2 ? P1_gpu : _get_density_matrix_gpu(H2, ϵF, P_method, Ncheb, maxdim, cutoff,
                                                            purify_method, purify_maxdim, purify_maxiters,
                                                            purify_tol, verbose)

    out_sites = [Index(dim(s), "Bubble,n=$i") for (i, s) in enumerate(H1.sites)]

    # ── Cache T_m·P1 for each active m, T_n·P2 for each active n ────────────
    verbose && println("ward_gpu: caching T_m·P and T_n·P on GPU...")
    A_cache = Dict{Int,MPO}()        # A_cache[m] = T_m · P1
    for m in active_m_set
        isnothing(Tn1[m]) && continue
        A_cache[m] = ITensorMPS.truncate!(
            apply(Tn1[m], P1_gpu; maxdim=maxdim, cutoff=cutoff); cutoff=cutoff)
        _gpu_gc!()
    end
    E_cache = H1 === H2 ? A_cache : Dict{Int,MPO}()   # E_cache[n] = T_n · P2
    if H1 !== H2
        for n in active_n_set
            isnothing(Tn2[n]) && continue
            E_cache[n] = ITensorMPS.truncate!(
                apply(Tn2[n], P2_gpu; maxdim=maxdim, cutoff=cutoff); cutoff=cutoff)
            _gpu_gc!()
        end
    end

    # ── Build QFT operators once ──────────────────────────────────────────────
    verbose && println("ward_gpu: building QFT operators (CPU → GPU)...")
    FTirev_gpu, FTrev_gpu = _build_qft_ops_gpu(H1)
    v_q_gpu = _to_gpu_mps(v_q)

    # ── Per (m,n) pair: two independent Hadamard → QFT → inner products ───────
    # No weighted MPO sums are formed — f_mn = val_A − val_B is a scalar difference.
    # This avoids the densitymatrix NaN from accumulating many F32 MPOs.
    n_pairs  = length(active_pairs)
    verbose && println("ward_gpu: $n_pairs (m,n) pairs × 2 inner products (GPU)...")
    f_direct = Dict{Tuple{Int,Int},ComplexF64}()
    t0_inner = time()

    for (idx, (m, n)) in enumerate(active_pairs)
        (isnothing(Tn1[m]) || isnothing(Tn2[n]) ||
         !haskey(A_cache, m) || !haskey(E_cache, n)) && continue

        # (T_m·P1) ⊙ T_n  →  QFT sandwich  →  ⟨v_q|·|v_q⟩
        had_A = _hadamard_mpo_gpu(A_cache[m], Tn2[n], out_sites; maxdim=maxdim, cutoff=cutoff)
        D_A   = _apply_qft_conj_gpu(replace_sites(had_A, H1.sites), FTirev_gpu, FTrev_gpu;
                                     tol=qft_tol, maxdim=qft_maxdim)
        val_A = ComplexF64(inner(v_q_gpu, D_A, v_q_gpu))
        _gpu_gc!()

        # T_m ⊙ (T_n·P2)  →  QFT sandwich  →  ⟨v_q|·|v_q⟩
        had_B = _hadamard_mpo_gpu(Tn1[m], E_cache[n], out_sites; maxdim=maxdim, cutoff=cutoff)
        D_B   = _apply_qft_conj_gpu(replace_sites(had_B, H1.sites), FTirev_gpu, FTrev_gpu;
                                     tol=qft_tol, maxdim=qft_maxdim)
        val_B = ComplexF64(inner(v_q_gpu, D_B, v_q_gpu))
        _gpu_gc!()

        f_direct[(m, n)] = val_A - val_B

        if verbose && (idx % 20 == 0 || idx == n_pairs)
            println("  ($m,$n) [$idx/$n_pairs, $(round(time()-t0_inner; digits=1))s]")
        end
    end

    # ── Per-ω accumulation (CPU scalars only) ────────────────────────────────
    σ = zeros(ComplexF64, nω)
    for (iω, ω) in enumerate(ωlist)
        abs(ω) < 1e-14 && continue
        chi_q = sum(C_all[iω][m, n] * get(f_direct, (m, n), zero(ComplexF64))
                    for (m, n) in active_pairs; init=zero(ComplexF64))
        σ[iω] = chi_q * ω / q_sq_val
    end

    verbose && println("ward_gpu: done.")
    return σ
end


"""
    get_conductivity_cheb2d_gpu(H1, H2, ωlist; kwargs...) -> Vector{Float64}

GPU-accelerated version of `get_conductivity_cheb2d`.
Same GPU/CPU split as `get_conductivity_ward_gpu`.
"""
function get_conductivity_cheb2d_gpu(H1::TBHamiltonian, H2::TBHamiltonian,
                                      ωlist::AbstractVector{<:Real};
                                      mu_a::Symbol          = :x,
                                      mu_b::Symbol          = :x,
                                      Ncheb::Int            = 50,
    ...
    _check_gpu("get_conductivity_cheb2d_gpu")
    H1.geometry === nothing && error("get_conductivity_cheb2d_gpu: H1.geometry is not set")
    H2.geometry === nothing && error("get_conductivity_cheb2d_gpu: H2.geometry is not set")
    nω = length(ωlist)
    ...
    # Current vertex (CPU — built once, not on the hot path)
    verbose && println("conductivity_cheb2d_gpu: building current vertex...")
    j_b  = make_current_operator_mpo(H1,
               make_displacement_mpo(H1, mu_b; tol=tol_dr);
               maxdim=maxdim, cutoff=cutoff)
    j_a  = (mu_a == mu_b) ? j_b :
           make_current_operator_mpo(H1,
               make_displacement_mpo(H1, mu_a; tol=tol_dr);
               maxdim=maxdim, cutoff=cutoff)
    out_sites_jab = [Index(dim(s), "Jab,n=$i") for (i, s) in enumerate(H1.sites)]
    J_ab = _hadamard_mpo(transpose_mpo(j_a), j_b, out_sites_jab;
                          maxdim=maxdim, cutoff=cutoff)
    J_ab = replace_sites(J_ab, H1.sites)
    ITensorMPS.truncate!(J_ab; cutoff=cutoff, maxdim=maxdim)

    # 2D GF + Tucker (CPU) — done first to derive active Tn indices
    N = Ncheb + 1
    C_all = [chebyshev2d_gf_coeffs(ω, scale1, center1, scale2, center2, η, N)
             for ω in ωlist]
    if kernel == :jackson
        ...
        C_all = [G_jk .* C for C in C_all]
    elseif kernel != :none
        error("get_conductivity_cheb2d_gpu: unknown kernel=$kernel")
    end

    T1 = hcat(C_all...); T2 = hcat([transpose(C) for C in C_all]...)
    F1 = svd(T1);        F2 = svd(T2)
    r_m = min(tucker_maxrank, sum(F1.S .> tucker_tol * F1.S[1]))
    r_n = min(tucker_maxrank, sum(F2.S .> tucker_tol * F2.S[1]))
    U_m = F1.U[:, 1:r_m]; V_n = F2.U[:, 1:r_n]
    for _ in 1:hooi_iters
        Y = hcat([C * V_n  for C in C_all]...); U_m = svd(Y).U[:, 1:r_m]
        Z = hcat([C' * U_m for C in C_all]...); V_n = svd(Z).U[:, 1:r_n]
    end
    verbose && println("conductivity_cheb2d_gpu: Tucker r_m=$r_m, r_n=$r_n → $(r_m*r_n) pairs")

    row_norms_m = [norm(U_m[m, :]) for m in 1:N]
    row_norms_n = [norm(V_n[n, :]) for n in 1:N]
    active_m = Set(m for m in 1:N if row_norms_m[m] > tucker_tol * maximum(row_norms_m))
    active_n = H1 === H2 ? active_m :
               Set(n for n in 1:N if row_norms_n[n] > tucker_tol * maximum(row_norms_n))
    verbose && println("conductivity_cheb2d_gpu: active Tns: $(length(active_m))/$N (H1)" *
                       (H1 === H2 ? " [reused for H2]" : ", $(length(active_n))/$N (H2)"))

    # Chebyshev moments on GPU (only active indices stored)
    verbose && println("conductivity_cheb2d_gpu: Chebyshev recurrence on GPU (Ncheb=$Ncheb)...")
    Tn1, _, _ = KPM_Tn_gpu(H1.mpo, Ncheb, H1.sites;
                            scale=scale1, center=center1,
                            maxdim=maxdim, cutoff=cutoff,
                            keep_indices=active_m, verbose=false)
    Tn2 = H1 === H2 ? Tn1 : KPM_Tn_gpu(H2.mpo, Ncheb, H2.sites;
                                         scale=scale2, center=center2,
                                         maxdim=maxdim, cutoff=cutoff,
                                         keep_indices=active_n, verbose=false)[1]

    # Density matrices: CPU purification → GPU
    verbose && println("conductivity_cheb2d_gpu: density matrices (CPU → GPU)...")
    P1_gpu = _get_density_matrix_gpu(H1, ϵF, P_method, Ncheb, maxdim, cutoff,
                                      purify_method, purify_maxdim, purify_maxiters,
                                      purify_tol, verbose)
    P2_gpu = H1 === H2 ? P1_gpu : _get_density_matrix_gpu(H2, ϵF, P_method, Ncheb, maxdim, cutoff,
    ...
    for iω in 1:nω; A_core[:, :, iω] = U_m' * C_all[iω] * V_n; end

    # Tucker MPO components on GPU
    verbose && println("conductivity_cheb2d_gpu: Tucker components (GPU)...")
    C_tuck, B_tuck, A_tuck, E_tuck = _build_tucker_components_gpu(
        Tn1, Tn2, P1_gpu, P2_gpu;
        U_m=U_m, V_n=V_n, r_m=r_m, r_n=r_n,
        maxdim=maxdim, cutoff=cutoff)

    # ω-independent Hadamard (GPU) → back to CPU → store D_tuck
    n_pairs = r_m * r_n
    verbose && println("conductivity_cheb2d_gpu: $n_pairs Hadamard products (GPU → CPU)...")
    ...
    for s1 in 1:r_m, s2 in 1:r_n
        ...
        had_A = _hadamard_mpo_gpu(A_tuck[s1], B_tuck[s2], out_sites;
                                   maxdim=maxdim, cutoff=cutoff)
        had_B = _hadamard_mpo_gpu(C_tuck[s1], E_tuck[s2], out_sites;
                                   maxdim=maxdim, cutoff=cutoff)
        D_gpu = ITensorMPS.truncate!(+(had_A, -1f0 * had_B; maxdim=maxdim); cutoff=cutoff)
        _gpu_gc!()

        D_tuck[s1, s2] = replace_sites(_to_cpu_mpo(D_gpu), H1.sites)

        if verbose
            idx = (s1 - 1) * r_n + s2
            (idx % 10 == 0 || idx == n_pairs) &&
                println("  ($s1,$s2)/($r_m,$r_n) [$idx/$n_pairs]")
        end
    end

    # Per-ω accumulation (CPU)
    coeff_tol_val = 1e-12
    Π = Vector{Union{Nothing,MPO}}(nothing, nω)
    for iω in 1:nω
        for s1 in 1:r_m, s2 in 1:r_n
            g = A_core[s1, s2, iω]
            (abs(g) < coeff_tol_val || isnothing(D_tuck[s1, s2])) && continue
            if Π[iω] === nothing
                Π[iω] = g * D_tuck[s1, s2]
            else
    ...
    σ = zeros(Float64, nω)
    for (iω, (ω, Pi0)) in enumerate(zip(ωlist, Π))
        abs(ω) < 1e-14 && continue
        isnothing(Pi0) && continue
        σ[iω] = -imag(inner(J_ab, Pi0)) / ω
    end

    verbose && println("conductivity_cheb2d_gpu: done.")
    return σ
end
