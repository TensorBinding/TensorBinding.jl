...
#
# Line 4 of the data file records the git tree hash of src/ and the last commit
# that changed src/, so a rerun after test-only commits reproduces the file
# byte for byte.
#
...
# The src/ tree hash and the last commit that changed src/: neither moves when
# only tests or docs are committed, so line 4 of the data file stays the same.
function src_info()
    try
        repo   = normpath(joinpath(@__DIR__, "..", ".."))
        tree   = readchomp(`git -C $repo rev-parse --short=12 HEAD:src`)
        commit = readchomp(`git -C $repo log -1 --abbrev=12 --format=%h -- src`)
        dirty  = !isempty(readchomp(`git -C $repo status --porcelain -- src`))
        return "src/ tree $tree (last changed in $commit)", dirty
    catch
        return "an unknown src/ tree (not a git checkout)", false
    end
end

function write_golden(path, entries, notes)
    source, dirty = src_info()
    open(path, "w") do io
        ...
        println(io, "# Generated from $source" * (dirty ? " with uncommitted changes under src/" : ""))
